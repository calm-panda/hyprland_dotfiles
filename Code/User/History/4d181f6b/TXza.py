...
async def getTsvZip(file_names: list[str]):
    ...
    tmp_dir_path = f"{ABS_PATH}/tmp_dir"
    # if path.exists()
...
if __name__ == "__main__":
    ...
    # new_metadata = new_metadata.groupby(new_metadata.columns[1]).agg(list).to_dict()[metadata_csv.columns[0]]
    # print(new_metadata)
    b = io.BytesIO()
    zf = zipfile.ZipFile(b, mode='w')
    zf.write("asb/folder to be saved.txt")
