...
# Genome ID (tsv's)#
####################
@app.post("/getGenomeIDs/", response_class=JSONResponse)
...
async def getFeatureFile(species_names: list[str], func_type: str):
    ...
    # get species list in lowercase "cazy"
    func_df = pd.read_csv(f"{ABS_PATH}/species_functionality_matrix_{func_type}.csv")
    req_df = data.loc[data[data.columns[0]].isin(species_names)]
    return 



if __name__ == "__main__":
    # Anaerotruncus_colihominis single
    data = pd.read_csv(f"{ABS_PATH}/species_functionality_matrix_cazy.csv")
    data.loc[data[data.columns[0]] == "Anaerotruncus_colihominis"].T