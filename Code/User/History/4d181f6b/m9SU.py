...
async def getFeatureFile(species_names: list[str], func_type: str):
    ...
    req_df = req_df.drop([req_df.columns[0]], axis=1)
    ...
    if len(species_names)==1:
        ...
        req_df = req_df.T
        req_df.to_csv(buffer, header=False)
    else:
    ...
    return StreamingResponse(
        iter([buffer.getvalue()]),
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment;filename={species_name if len(species_names)==1 else 'FunctionalProfile'}.csv",
                 "Content-Length": str(buffer.getbuffer().nbytes)}
    )
